...
# Rubrics duplicated here so the seed is self-contained and stable across
# refactors of app.services.judge_metrics.
...
def upgrade() -> None:
    ...
    # Resolve a creator for the seeded rows. We require created_by NOT NULL,
    # so pick the oldest user as a stable owner. If no users exist (fresh DB,
    # before any registration), skip seeding — the predefined metrics will be
    # inserted lazily at first list call by the service layer.
    user_id_row = bind.execute(
        sa.text("SELECT id FROM \"user\" ORDER BY id LIMIT 1")
    ).fetchone()
    if user_id_row is None:
        return

    seed_owner = user_id_row[0]
    ...
    existing_names = {
        ...
        for row in bind.execute(
            sa.text(
                "SELECT name FROM custom_metric WHERE deleted_at IS NULL"
            )
        ).fetchall()
    }
...
def downgrade() -> None:
    ...
    bind.execute(
        sa.text(
            "DELETE FROM custom_metric WHERE is_predefined = TRUE"
        )
    )
